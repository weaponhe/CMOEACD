...
public class ViolationThresholdComparator<S extends Solution<?>> implements
  ...
  protected OverallConstraintViolation<S> overallConstraintViolation ;
  protected NumberOfViolatedConstraints<S> numberOfViolatedConstraints ;
  ...
  public int compare(S solution1, S solution2) {
    ...
    overall1 = overallConstraintViolation.getAttribute(solution1) *  numberOfViolatedConstraints.getAttribute(solution1) ;
    overall2 = overallConstraintViolation.getAttribute(solution2) * numberOfViolatedConstraints.getAttribute(solution2) ;
  ...
  public boolean needToCompare(S solution1, S solution2) {
    ...
    double overall1, overall2;
    overall1 = Math.abs(overallConstraintViolation.getAttribute(solution1) * numberOfViolatedConstraints.getAttribute(solution1) );
    overall2 = Math.abs(overallConstraintViolation.getAttribute(solution2) * numberOfViolatedConstraints.getAttribute(solution2) );
  ...
  public boolean underViolationEp(S solution) {
    double overall;
    overall = Math.abs(overallConstraintViolation.getAttribute(solution)*numberOfViolatedConstraints.getAttribute(solution));
...
//      if (overallConstraintViolation.getAttribute(solutionSet.get(i)) < 0) {
  ...
  public double meanOverallViolation(List<S> solutionSet) {
    double aux = 0.0;
    for (int i = 0; i < solutionSet.size(); i++) {
      aux += Math.abs(overallConstraintViolation.getAttribute(solutionSet.get(i)) * numberOfViolatedConstraints.getAttribute(solutionSet.get(i)) );
    }
    return aux / (double) solutionSet.size();
  }
