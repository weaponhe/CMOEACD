...
public class ArrayFront implements Front {
  ...
  @Override public void copySetPoint(int index, Point point,int len) {
    if (index < 0) {
      throw new JMetalException("The index value is negative") ;
    } else if (index >= numberOfPoints) {
      throw new JMetalException("The index value (" + index + ") is greater than the number of "
    ...
    //points[index] = new ArrayPoint(point.getNumberOfDimensions());
    for(int i=0;i<len;i++)
      points[index].setDimensionValue(i,point.getDimensionValue(i));

  }

  @Override public void swap(int idx1,int idx2){
  ...
  @Override public void sort(Comparator<Point> comparator) {
    ...
    sort(0,numberOfPoints,comparator);
  }

  @Override public void sort(int idxFrom, int idxTo,Comparator<Point> comparator) {
    Arrays.sort(points, idxFrom, idxTo, comparator);
  }
