...
//            printData2(latexFile, i, min, median, max, "Best, Median and Worst");
        }
    ...
    private void printData2(String latexFile, int indicatorIndex, double[][][] min, double[][][] median, double[][][] max, String caption) throws IOException {
        // Generate header of the table
        FileWriter os = new FileWriter(latexFile, true);
        os.write("\n");
        os.write("\\begin{table}" + "\n");
        ...
        os.write("\\centering" + "\n");
        os.write("\\begin{scriptsize}" + "\n");
        os.write("\\begin{tabu} to 0.8\\linewidth{X[3,c]|X[1,c]");
        for (int i = 0; i < experiment.getAlgorithmList().size(); i++) {
            os.write("|X[2,c]");
        }
        os.write("}\n");
        os.write("\\toprule" + "\n");
        os.write("\\textbf{Test Instance} & \\textbf{m} ");


        for (int i = 0; i < experiment.getAlgorithmList().size(); i++) {
            os.write("&  \\textbf{" + experiment.getAlgorithmList().get(i).getTag() + "}");
        }
        os.write("\n");
        ...
        // write lines
        for (int i = 0; i < experiment.getProblemList().size(); i++) {
            os.write("\\multirow{15}*{\\begin{sideways} \\textbf{"+experiment.getAlgorithmList().get(i).getTag()+"}\\end{sideways}}" + "\n");
        }
        // close table
        os.write("\\end{tabu}" + "\n");
        os.write("\\end{scriptsize}" + "\n");
