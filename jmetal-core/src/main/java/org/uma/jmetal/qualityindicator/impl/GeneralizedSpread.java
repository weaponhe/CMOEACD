...
public class GeneralizedSpread<S extends Solution<?>> extends GenericIndicator<S> {
  ...
  public GeneralizedSpread() {
    setName("GSPREAD");
    setDescription("Generalized Spread quality indicator");
  }
  ...
  public double generalizedSpread(Front front, Front referenceFront) {
    ...
    if (new EuclideanDistance().compute(front.getPoint(0),
      ...
      double[] nearestDist = new double[front.getNumberOfPoints()];
      for (int i = 0 ; i < front.getNumberOfPoints(); i++) {
        nearestDist[i] = FrontUtils.distanceToNearestPoint(front.getPoint(i), front);
        dmean += nearestDist[i];
      }
      ...
      double mean = 0.0;
      for (int i = 0; i < front.getNumberOfPoints(); i++) {
//        FrontUtils.distanceToNearestPoint(front.getPoint(i), front)
        mean += Math.abs( nearestDist[i] - dmean);
      }
