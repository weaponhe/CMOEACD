 ...
 */
public class MOEADACDN extends MOEADACD {
    protected double[] intercepts;
    protected double[] normIntercepts;
    public MOEADACDN(Problem<DoubleSolution> problem, int populationSize, int resultPopulationSize, int maxEvaluations,
    ...
    protected void updateTheta(){
        ...
        for(int i=0;i<populationSize;++i){
            for(int j=0;j<problem.getNumberOfObjectives();++j)
                normObjectives[j] = (population.get(i).getObjective(j) - idealPoint[j])/normIntercepts[j];

            tmp[i] = MOEADUtils.angle(lambda[i],normObjectives);
            sumTmp += tmp[i];
        }
        for(int i=0;i<populationSize;++i){
