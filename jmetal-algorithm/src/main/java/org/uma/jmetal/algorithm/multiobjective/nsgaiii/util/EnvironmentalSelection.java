...
//			int i ;
//			for( i = base;i<N;++i){
//				if(Math.abs(A.get(i).get(base)) > 1e-10)
//					break;
//			}
//			if(i == N)
//				continue;
//			if(i > base){
//				double tmp ;
//				for(int term = base;term<A.get(i).size();term++){
//					tmp = A.get(base).get(term);
//					A.get(base).set(term,A.get(i).get(term));
//					A.get(i).set(term,tmp);
//				}
//			}
...
//		List<Double>   intercepts = new ArrayList<>(numberOfObjectives);
//		for(int i=0;i<numberOfObjectives;++i)
//			intercepts.add(1.0);
