 ...
 */
public class CMOEACDO extends CMOEACD {
    protected List<DoubleSolution> optima;
    public CMOEACDO(Problem<DoubleSolution> problem,
                     int[] arrayH,
    ...
    public CMOEACDO(Measurable measureManager, Problem<DoubleSolution> problem,
                     int[] arrayH,
                     double[] integratedTaus,
                     int populationSize,
                     int maxEvaluations,
...
//            associateSubRegionWithOptima(optima,utopianPoint,normIntercepts);
    ...
    protected CMOEACDA.POPTYPE randomSelectedPolutionType(){
        double pp = randomGenerator.nextDouble(0.0,1.0);
        if(pp < 0.75) {
            selectedPopType = CMOEACDA.POPTYPE.POP;
        }else {
            selectedPopType = CMOEACDA.POPTYPE.OPTIMUM;
        }
    ...
    protected List<DoubleSolution> parentSelection(int idxSubRegion,int parentPoolSize) {
        ...
        if(selectedPopType == CMOEACDA.POPTYPE.POP){
        ...
        }else if(selectedPopType == CMOEACDA.POPTYPE.OPTIMUM){
