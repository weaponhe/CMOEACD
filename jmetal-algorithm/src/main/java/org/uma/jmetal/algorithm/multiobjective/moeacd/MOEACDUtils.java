...
//        JMetalLogger.logger.info("decode");
    ...
    //0: non-dominate
    //-1: ind1 dominate ind2
    //1: ind2 dominate ind1
    //2: unkown
    ...
    public static int constraintDominateCompare(DoubleSolution ind1, DoubleSolution ind2) {
        double cv1 = (double) ind1.getAttribute("overallConstraintViolationDegree");
        double cv2 = (double) ind2.getAttribute("overallConstraintViolationDegree");
        boolean isFessible1 = cv1 >= 0;
        boolean isFessible2 = cv2 >= 0;
        if (isFessible1 && isFessible2) {
            return dominateCompare(ind1, ind2);
        } else if (isFessible1) {
            return -1;
        } else if (isFessible2) {
            return 1;
        } else {
            return Math.abs(cv1) < Math.abs(cv2) ? -1 : 1;
        }
    }
