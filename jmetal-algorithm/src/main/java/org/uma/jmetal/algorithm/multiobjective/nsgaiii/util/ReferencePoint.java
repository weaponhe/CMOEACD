...
public class ReferencePoint<S extends Solution<?>> {
  ...
  public void generateReferencePoints(
          ...
          double[] intergratedTaus) {

    ReferencePoint<S> refPoint = new ReferencePoint<>(numberOfObjectives) ;
    for(int i=0;i<numberOfDivisions.length;++i) {
      generateRecursive(referencePoints, refPoint, numberOfObjectives, numberOfDivisions[i], numberOfDivisions[i], 0, intergratedTaus[i]);
    }
  }
  ...
  private void generateRecursive(
          ...
          int left,
          int total,
          int element,double tau) {
    if (element == (numberOfObjectives - 1)) {
      refPoint.position.set(element, (double) left / total) ;
//      //modified by Yuehong Xie
//      refPoint.position.set(element,(1.0 - tau)/numberOfObjectives + tau * refPoint.position.get(element));
      ...
      for (int i = 0 ; i <= left; i +=1) {
        ...
        generateRecursive(referencePoints, refPoint, numberOfObjectives, left-i, total, element+1,tau);
      }
    }
  }
