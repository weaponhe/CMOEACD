...
//    constraint[0] = Math.max(maxSum1, sum2) ;
    ...
    double sum2 = 0 ;
    double minSum1 = Double.MAX_VALUE;
    double sumObj = 0.0;
    for (int i=0;i<getNumberOfObjectives();i++){
      sumObj += Math.pow(solution.getObjective(i),2.0);
    }

    for (int i = 0; i < getNumberOfObjectives(); i++) {
      double sum1 = Math.pow(solution.getObjective(i)-1.0, 2.0) + sumObj - Math.pow(solution.getObjective(i),2.0) -  Math.pow(rValue, 2.0) ;
      minSum1 = Math.min(minSum1, sum1) ;

      sum2 += Math.pow((solution.getObjective(i) -
              1.0/Math.sqrt(1.0*getNumberOfObjectives())), 2.0)  ;
    }

    sum2 -= Math.pow(rValue, 2.0) ;

    constraint[0] = -1.0 * Math.min(minSum1, sum2) ;
