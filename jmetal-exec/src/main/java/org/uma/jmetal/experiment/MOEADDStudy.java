        ...
        for(int iProblem=0;iProblem<problemList.size();++iProblem){
            ...
            for(int iV = 0;iV<variantConfig.length;++iV) {
                ...
                for (int iRun = 0; iRun < maxRun; ++iRun) {
                    ...
                    AbstractMOEAD<DoubleSolution> algorithm = new MOEADBuilder(problemList.get(iProblem), MOEADBuilder.Variant.MOEADDMeasure)
                ...
                /* Measure management */
                    MeasureManager measureManager = ((MOEADDMeasure) algorithm).getMeasureManager();
...
//                AbstractMOEAD.FunctionType.TCH,
        ...
        for(int iProblem=0;iProblem<problemList.size();++iProblem){
            ...
            for(int iV = 0;iV<variantConfig.length;++iV) {
                ...
                for (int iRun = 0; iRun < maxRun; ++iRun) {
                    JMetalLogger.logger.info("[" + variantName[iV] +"--"+ problemList.get(iProblem).getName() +"(" + problemList.get(iProblem).getNumberOfObjectives() + ")]  Run : " + iRun);
                    //configure the algorithm
                    AbstractMOEAD<DoubleSolution> algorithm = new MOEADBuilder(problemList.get(iProblem), MOEADBuilder.Variant.MOEADD)
                            .setCrossover(sbxCrossover)
                            .setMutation(mutation)
                            .setMaxEvaluations(maxEvaluations)
                            .setPopulationSize(popsList[iProblem])
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            moeaddExperiment.execute("E:/Results/",
