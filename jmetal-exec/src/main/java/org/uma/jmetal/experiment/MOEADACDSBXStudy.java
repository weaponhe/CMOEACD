...
import org.uma.jmetal.algorithm.multiobjective.moead.MOEADACDSBXMeasure;
        ...
        for(int iProblem=0;iProblem<problemList.size();++iProblem){
            ...
            for(int iV = 0;iV<variantConfig.length;++iV) {
                ...
                for (int iRun = 0; iRun < maxRun; ++iRun) {
                    ...
                    AbstractMOEAD<DoubleSolution> algorithm = new MOEADBuilder(problemList.get(iProblem), MOEADBuilder.Variant.MOEADACDSBXMeasure)
                ...
                /* Measure management */
                    MeasureManager measureManager = ((MOEADACDSBXMeasure) algorithm).getMeasureManager();
        ...
        for(int iProblem=0;iProblem<problemList.size();++iProblem){
            ...
            for(int iV = 0;iV<variantConfig.length;++iV) {
                ...
                for (int iRun = 0; iRun < maxRun; ++iRun) {
                    JMetalLogger.logger.info("[" + variantName[iV] +"--"+ problemList.get(iProblem).getName() +"(" + problemList.get(iProblem).getNumberOfObjectives() + ")]  Run : " + iRun);
                    //configure the algorithm
                    AbstractMOEAD<DoubleSolution> algorithm = new MOEADBuilder(problemList.get(iProblem), MOEADBuilder.Variant.MOEADACDSBX)
                            .setCrossover(sbxCrossover)
                            .setMutation(mutation)
                            .setMaxEvaluations(maxEvaluations)
                            .setPopulationSize(popsList[iProblem])
