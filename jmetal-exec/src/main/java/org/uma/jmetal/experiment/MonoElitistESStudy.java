 ...
 */
public class MonoElitistESStudy {
    public void executeMeasure(String baseDir,
        ...
        for(int iProblem=0;iProblem<problemList.size();++iProblem) {
            ...
            String instance = "ElitistES_" + problemList.get(iProblem).getName() + "(" + problemList.get(iProblem).getNumberOfObjectives() + ")_" + popsList[iProblem] + "_" + maxIterationsList[iProblem];
            MyExperimentMonoAnalysis experimentAnalysis = new MyExperimentMonoAnalysis(baseDir, instance);

            for (int iRun = 0; iRun < maxRun; ++iRun) {
                JMetalLogger.logger.info("[ElitistES--" + problemList.get(iProblem).getName() + "(" + problemList.get(iProblem).getNumberOfObjectives() + ")]  Run : " + iRun);
                //configure the algorithm
                ElitistEvolutionStrategyMeasure algorithm =(ElitistEvolutionStrategyMeasure) new EvolutionStrategyMeasureBuilder((DoubleProblem) problemList.get(iProblem), EvolutionStrategyMeasureBuilder.EvolutionStrategyVariant.ELITIST)
                ...
                /* Measure management */
                MeasureManager measureManager = ((ElitistEvolutionStrategyMeasure) algorithm).getMeasureManager();
