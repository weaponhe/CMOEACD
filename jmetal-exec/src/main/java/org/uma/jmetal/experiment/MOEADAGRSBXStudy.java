...
import org.uma.jmetal.algorithm.multiobjective.moead.MOEADAGRSBXMeasure;
import org.uma.jmetal.algorithm.multiobjective.moead.MOEADBuilder;
import org.uma.jmetal.algorithm.multiobjective.moead.MOEADGRSBXMeasure;
...
/**
 * Created by X250 on 2016/9/13.
 */
public class MOEADAGRSBXStudy {
        ...
        for(int iProblem=0;iProblem<problemList.size();++iProblem){
            ...
            for(int iV = 0;iV<variantConfig.length;++iV) {
                ...
                for (int iRun = 0; iRun < maxRun; ++iRun) {
                    ...
                    AbstractMOEAD<DoubleSolution> algorithm = new MOEADBuilder(problemList.get(iProblem), MOEADBuilder.Variant.MOEADAGRSBXMeasure)
                ...
                /* Measure management */
                    MeasureManager measureManager = ((MOEADAGRSBXMeasure) algorithm).getMeasureManager();
        ...
        for(int iProblem=0;iProblem<problemList.size();++iProblem){
            ...
            for(int iV = 0;iV<variantConfig.length;++iV) {
                ...
                for (int iRun = 0; iRun < maxRun; ++iRun) {
                    JMetalLogger.logger.info("[" + variantName[iV] +"--"+ problemList.get(iProblem).getName() +"(" + problemList.get(iProblem).getNumberOfObjectives() + ")]  Run : " + iRun);
                    //configure the algorithm
                    AbstractMOEAD<DoubleSolution> algorithm = new MOEADBuilder(problemList.get(iProblem), MOEADBuilder.Variant.MOEADAGRSBX)
                            .setCrossover(sbxCrossover)
                            .setMutation(mutation)
                            .setMaxEvaluations(maxEvaluations)
                            .setPopulationSize(popsList[iProblem])
